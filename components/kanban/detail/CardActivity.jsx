...
import { List, Loader, Send, CheckSquare, Trash2 } from 'lucide-react';
  ...
  return (
    <section>
      <div className="space-y-8">
        <div id="card-subtasks-section" className="space-y-4">
          <h4 className="text-sm font-bold text-[var(--text-heading)]">Subtasks {subtasks.length > 0 && `(${completedSubtasks}/${subtasks.length})`}</h4>
          {subtasksContent}
        </div>
        <div className="space-y-4 border-t border-[var(--border-subtle)] pt-6">
          <h4 className="text-sm font-bold text-[var(--text-heading)]">Comments {comments.length > 0 && `(${comments.length})`}</h4>
          {commentsContent}
        </div>
      </div>
    </section>
