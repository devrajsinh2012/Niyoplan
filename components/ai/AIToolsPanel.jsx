...
export default function AIToolsPanel({ projectId }) {
  ...
  return (
    <div className="space-y-6">
      <section className="glass-panel rounded-2xl p-6 bg-slate-900/50 border border-slate-800 space-y-4">
        <h3 className="text-white text-lg font-bold">AI Assistant Toolkit</h3>
        <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider bg-slate-800/50 px-3 py-1 rounded-full inline-block">
          Context: {cards.length} cards · {sprints.length} sprints · {meetings.length} meetings
        </div>
        
        <div className="space-y-3">
          <input 
            className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3.5 text-white focus:border-blue-500 outline-none text-sm font-semibold shadow-inner" 
            placeholder="Ticket title" 
            value={title} 
            onChange={(e) => setTitle(e.target.value)} 
          />
          <textarea 
            className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3.5 text-slate-300 focus:border-blue-500 outline-none text-sm font-medium min-h-[120px] shadow-inner leading-relaxed" 
            placeholder="Description / context" 
            value={description} 
            onChange={(e) => setDescription(e.target.value)} 
          />
          <textarea 
            className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3.5 text-slate-300 focus:border-blue-500 outline-none text-sm font-medium min-h-[96px] shadow-inner leading-relaxed" 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2.5 pt-2">
          <button className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 rounded-xl px-4 py-3 text-[11px] font-bold uppercase tracking-wider transition-all active:scale-95 disabled:opacity-50" disabled={loading} onClick={() => runTool('/api/ai/generate-description', { title, context: description }, 'description')}>Generate Description</button>
      ...
      <section className="glass-panel rounded-2xl p-6 bg-slate-900/50 border border-slate-800">
        <h4 className="text-white font-bold text-base mb-4">AI Intelligence Output</h4>
        <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-5 min-h-[224px] text-xs text-slate-300 whitespace-pre-wrap leading-relaxed font-medium shadow-inner">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-3">
              <div className="w-8 h-8 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin" />
              <span className="text-slate-500 font-bold uppercase tracking-widest text-[10px]">Processing Intelligence...</span>
            </div>
          ) : (
